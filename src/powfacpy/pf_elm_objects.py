...
class Line(PFElm):
    ...
    def get_P_res_var_lf_bal(self, bus_index: int | slice = slice(0, None)):
        return [lf_bal.ElmLne.m_Psum_bus1.value, lf_bal.ElmLne.m_Psum_bus2.value][
            bus_index
        ]

    def get_Q_res_var_lf_bal(self, bus_index: int | slice = slice(0, None)):
        return [lf_bal.ElmLne.m_Qsum_bus1.value, lf_bal.ElmLne.m_Qsum_bus2.value][
            bus_index
        ]

    def get_P_res_var_rms_bal(self, bus_index: int | slice = slice(0, None)):
        return [rms_bal.ElmLne.m_Psum_bus1.value, rms_bal.ElmLne.m_Psum_bus2.value][
            bus_index
        ]

    def get_Q_res_var_rms_bal(self, bus_index: int | slice = slice(0, None)):
        return [rms_bal.ElmLne.m_Qsum_bus1.value, rms_bal.ElmLne.m_Qsum_bus2.value][
            bus_index
        ]
...
class Transformer2Winding(PFElm):
    ...
    def get_P_res_var_rms_bal(self, bus_index: int | slice = slice(0, None)):
        return [rms_bal.ElmTr2.m_Psum_bushv.value, rms_bal.ElmTr2.m_Psum_buslv][
            bus_index
        ]

    def get_Q_res_var_rms_bal(self, bus_index: int | slice = slice(0, None)):
        return [rms_bal.ElmTr2.m_Qsum_bushv.value, rms_bal.ElmTr2.m_Qsum_buslv.value][
            bus_index
        ]
...
class Transformer3Winding(PFElm):
    ...
    def get_connected_elms_ordered(self) -> list[PFGeneral]:
        return [
            self.pf_obj.bushv.cterm,
            self.pf_obj.busmv.cterm,
            self.pf_obj.buslv.cterm,
        ]
