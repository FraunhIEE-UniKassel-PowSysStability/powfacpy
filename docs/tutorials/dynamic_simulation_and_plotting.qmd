...
::: {.callout-note}
`plot_from_pandas_using_comtrade` needs the `comtraderecord` package, which is an optional dependency of *powfacpy*. Install it with `pip install powfacpy[comtrade]` (or `pip install comtraderecord`) before running this section.
:::
...
# remove the csv file. This cell will not be shown after rendering.
import os
...
pfres.export_to_csv(
    dir=r".\output\dynamic_simulation_and_plotting",
...
csv_path = r".\output\dynamic_simulation_and_plotting\Power of Synchronous Machine 01.csv"
plot = Plots.plot_from_csv(csv_path, r"G 01\s:P1")
plot = Plots.plot_from_csv(csv_path, r"G 01\s:Q1")
plt.grid()
```

*Pyplot* is used by default, but you can specify a different plot interface using the optional argument `plot_interface`.

# Exporting Curves of Plot to Pandas DataFrame
It is also possible to directly export curves from a plot to a pandas DataFrame. This is useful if you have created a plot in *PowerFactory* and want to analyze the data further using pandas.

```{python}
pfplt.set_active_plot("Plot 1", "Plot page 1")
pfplt.export_curves_to_pandas_dataframe()
```
