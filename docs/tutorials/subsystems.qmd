...
export_dsl_model_info_to_csv(governor_info, r".\tests\tests_output\governor_info")
```

There is also a method to export the information on all dynamic models (governors, AVRs, PSS) of the subsystem to csv files:

```{python}
subsystems[0].dynamic_models.synchronous_machines.export_info_to_csv(r".\tests\tests_output\synchronous_machines_info")
```
...
```{python}
ss_container.export_synchronous_machines_info_to_csv(r".\tests\tests_output\synchronous_machines_info_by_subsystem")
```
