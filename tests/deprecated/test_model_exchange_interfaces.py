...
def _compare_loadflow_results(before:dict, after:dict, THRESHOLD=1e-4):
    for element_name, value_before in before.items():
        before_after_difference = abs((value_before - after[element_name])/value_before)
...
def test_cgmes_update_profiles(pfcgmes, activate_test_project):
    ...
    study_case_source = pfcgmes.get_unique_obj(r"Study Cases\test_model_exchange_interfaces\Study Case")
    ...
    base_archive = pfcgmes.get_unique_obj(r'cgmes_archive_folder\cgmes_archive_imported')
    pfcgmes.update_profiles(
        OUTPUT_PATH + '\\' + pfcgmes.EXPORTED_ZIP_NAME + '.zip',
        base_archive    
    )
