...
def test_get_obj_with_parent_folder_argument(pfp, activate_test_project):
    ...
    grid = pfp.get_obj(
        "Grid", parent_folder=r"Network Model\Network Data\test_active_project_interface")[0]
...
def test_set_attr(pfp, activate_test_project):
    ...
    pfp.set_attr(r"Library\Dynamic Models\Linear_interpolation",
                 {"sTitle": test_string_1})
    pfp.set_attr("Linear_interpolation", {"sTitle": test_string_2,
                                          "desc": ["dummy description"]}, parent_folder=r"Library\Dynamic Models")
    stitle = pfp.get_attr(
        r"Library\Dynamic Models\Linear_interpolation", "sTitle")
...
def test_copy_obj(pfp, activate_test_project):
    ...
    pfp.delete_obj("*", parent_folder=folder_copy_to,
                   error_if_non_existent=False)
    copied_objects = pfp.copy_obj(
    ...
    copied_objects = pfp.copy_obj(
        objects_to_copy, folder_copy_to, overwrite=False)
    assert len(copied_objects) == 2
...
def test_copy_single_obj(pfp, activate_test_project):
    ...
    obj_to_copy = pfp.get_single_obj(
        "dummy2.*", parent_folder=folder_copy_from)
    copied_object = pfp.copy_single_obj(
        obj_to_copy, folder_copy_to, overwrite=True)
...
def test_replace_outside_or_inside_of_strings_in_a_string(pfp, activate_test_project):
    ...
    assert (conditions ==
            "lorem ipsum x[1] == 'ABC control 1' 'control 1' x[1]")
